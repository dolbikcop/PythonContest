from Docs import Translator, Salary, Vacancy, UI
from unittest import TestCase

class UserInterfaceTests(TestCase):
    def test_user_interface_type(self):
        self.assertEqual(type(UI()).__name__, 'UI')

    def test_user_interface_default_profession_name(self):
        self.assertEqual(UI().prof_name, 'Программист')

    def test_user_interface_profession_name(self):
        self.assertEqual(UI(prof_name='Аналитик').prof_name, 'Аналитик')

    def test_user_interface_default_file_name(self):
        self.assertEqual(UI().f_name, '../vacancies_medium.csv')

    def test_user_interface_file_name(self):
        self.assertEqual(UI(file_name='../vacancies_by_year.csv').f_name, '../vacancies_by_year.csv')
...
class TranslatorTests(TestCase):
    ...
    def test_translator_method_translate(self):
        self.assertEqual(Translator().to_russian('USD'), 'Доллары')
