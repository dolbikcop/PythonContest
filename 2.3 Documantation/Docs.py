...
class Report:
    ...
    def draw_bar_graph(self, subplot, name: str) -> None:
        ...
        second_label = f'з/п {ui.prof_name}'
    ...
    def generate_pdf(self, name: str):
        ...
        pdf_template = template.render(
            {'image_file': image_file,
             ...
             'profession_name': f"{ui.prof_name}",
...
class CSV:
    """
    Класс для обработки файлов с расширением CSV.
    Attributes
    ----------
    dataset : csv.reader
        Необработанные данные CSV-файла.
    header : list
        Список заголовков CSV-таблицы.
    rows : list
    ...
    def __init__(self, f_name: str):
        """
        Создаёт экземпляр класса. Пытается прочесть CSV-файл с переданным именем.
        :param f_name: Имя CSV-файла.
        """
        with open(f_name, 'r', newline='', encoding='utf-8-sig') as file:
            self.dataset = csv_reader(file)
            try:
                self.header = next(self.dataset)
            except StopIteration:
                custom_exit('Пустой файл')

            self.rows = [row for row in self.dataset
                         if len(list(filter(lambda word: word != '', row))) == len(self.header)]
...
class Salary:
    ...
    def set_field(self, key: str, value: str) -> None:
        ...
        if key == 'salary_currency':
            value = translator.to_russian(value)
    ...
    def get_average_in_rur(self) -> int:
        ...
        """
        return translator.to_ruble_currency(self.salary_currency) * (self.salary_from + self.salary_to) // 2
...
class Translator:
    """Класс для перевода разных валют по установленному курсу."""
    ...
    def to_russian(self, key: str, name_dict: str = None) -> str:
        """
        Переводит из международного формата на русский язык.

        :param key: Международный формат валюты или название словаря.
        :param name_dict: Имя словаря из аттрибутов класса для доступа к готовым значениям.
        """
        if name_dict is not None:
            return self.__getattribute__(name_dict)[key]
        return self.__getattribute__(key)

    def to_ruble_currency(self, cu: str) -> int or float:
        """
        Даёт курс некоторой валюты в рублях.
        :param cu: Название валюты на русском языке.
        """
        return self.to_ruble[cu]

class UI:
    """
    Класс для работы с введенными пользователем данными.
    Attributes
    ----------
    f_name : str
        Имя файла с расширением CSV.
    prof_name : str
        Название профессии для обработки.
    """
...
def custom_exit(message: str) -> None:
    """
    Оповещение в консоли и выход из программы.
    :param message: Сообщение в консли.
    """
